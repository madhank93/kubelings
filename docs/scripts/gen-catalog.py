...
MODULES = {
    ...
    "M2":  ("Workloads",            "#3b9eff",
            "rolling updates, blue/green & canary, DaemonSets, StatefulSets, Jobs, CronJobs, HPA, OOMKill & right-sizing, CPU throttling, probes, init containers, PDBs, QoS, ephemeral-container debugging, multi-container patterns, readiness/CronJob/rollout failure drills, VPA, KEDA, Job retry bounds"),
    "M3":  ("Config & Storage",     "#d29922",
            "ConfigMaps, Secrets, PV/PVC lifecycle, StorageClasses, access modes, finalizer traps, kustomize, Helm release lifecycle, ghost-endpoint & secret-rotation & stuck-namespace drills, databases on Kubernetes"),
    "M4":  ("Networking",           "#4fa86d",
            "Services & endpoints, Ingress & Gateway API, NetworkPolicy, CoreDNS & the ndots amplifier, kube-proxy dataplane, CNI anatomy & triage, conntrack, graceful shutdown, externalTrafficPolicy blackholes, pod-IP exhaustion, kubeconfig contexts"),
    ...
    "M6":  ("Security",             "#c53030",
            "RBAC, ServiceAccounts & tokens, Pod Security, admission webhooks, container hardening, CIS benchmarks, egress lockdown, image digests, Gatekeeper & Kyverno policy engines, trivy scanning, cosign signatures & SBOMs, seccomp/AppArmor, encryption-at-rest, audit policy, Falco runtime detection, cloud pod identity"),
    "M7":  ("Internals",            "#e36f0e",
    ...
    "M8":  ("Observability & SRE",  "#1f8a9c",
            "events forensics, node NotReady triage, quotas, disk pressure & eviction, cluster upgrades, node maintenance, SLO burn-rate alerting, OTel tracing pipelines, log-pipeline failures, debugging playbooks"),
    ...
    "M10": ("Platform Engineering", "#db6d28",
            "GitOps with Argo CD (incl. app-of-apps) and Flux, multi-tenancy with Capsule, Cluster API, Crossplane compositions, prune blast radius, service-mesh trade-offs"),
...
INCIDENTS = {
    ...
    "incident-job-restartpolicy":  ("Universe",     "http://status.universe.com/incidents/115n3vxqwzcf", "/incidents/universe-job-restartpolicy/"),
    "incident-gitops-prune":       ("Skyscanner",   "https://medium.com/@SkyscannerEng/how-a-couple-of-characters-brought-down-our-site-356ccaf1fbc3", "/incidents/skyscanner-gitops-prune/"),
    "incident-ip-exhaustion":      ("loveholidays", "https://deploy.live/blog/when-gke-ran-out-of-ip-addresses/", "/incidents/loveholidays-ip-exhaustion/"),
    "incident-iam-sidecar":        ("Adevinta",     "https://srvaroa.github.io/kubernetes/migration/latency/dns/java/aws/microservices/2019/10/22/kubernetes-added-a-0-to-my-latency.html", "/incidents/adevinta-iam-sidecar/"),
    "incident-istio-integration":  ("Exponea",      "https://medium.com/@jakubkulich/sailing-with-the-istio-through-the-shallow-water-8ae81668381e", "/incidents/exponea-istio-integration/"),
}
