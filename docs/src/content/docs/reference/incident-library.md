...
| Spotify | Accidentally deleted ALL kube clusters — no user impact | infra-as-code blast radius, DR | M9 capstone · reading `incident-spotify-delete` · [case study](/incidents/spotify-delete/) | [KubeCon talk](https://www.youtube.com/watch?v=ix0Tw8uinWs) |
...
| Target | Cascading failure of distributed systems (Kafka/Consul/Docker) | cascade thinking, resource stampedes | M9 capstone · reading `incident-target-cascade` · [case study](/incidents/target-cascade/) | [blog](https://medium.com/@daniel.p.woods/on-infrastructure-at-scale-a-cascading-failure-of-distributed-systems-7cff2a3cd2df) |
...
| Google Cloud | 2019-06-02 — maintenance automation descheduled the network control plane jobs; multi-region packet loss for ~4h | control planes have blast radius too, maintenance automation risk | M4/M7 | [incident report](https://status.cloud.google.com/incident/cloud-networking/19009) |
...
| Ghost endpoints after scale-down | graceful shutdown | M3 · drill `pattern-ghost-endpoints` | [read](/incidents/pattern-ghost-endpoints/) |
| Secret rotated, pods never noticed | mounts vs env, reload | M3 · drill `pattern-secret-not-reloaded` | [read](/incidents/pattern-secret-not-reloaded/) |
...
| Namespace stuck `Terminating` | finalizer removal, CRD lifecycle | M3 · drill `pattern-namespace-terminating` | [read](/incidents/pattern-namespace-terminating/) |
...
| Webhook timeout blocks the write path | failurePolicy, timeout tuning | M6/M7 | coming soon |
| Mutable image tag drift | immutable tags, digest pinning | M6 · runnable `image-digests` | coming soon |
| API server OOM under a watch storm | watch pressure, APF, list pagination | M7 | coming soon |
| etcd compaction stall | etcd maintenance, auto-compaction | M7 | coming soon |
